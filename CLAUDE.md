...
Inji Certify (`develop`, 1.0.0-beta.1) is an OpenID4VCI 1.0 credential issuer whose issuance, formatting, templating, signing and configuration are fused in one Spring Boot module. The rebuild re-layers it without changing what wallets or operators see: a protocol-agnostic core (`certify-core`) behind `certify-spi`, protocol adapters (`oid4vci-v1` from today's controllers, `oid4vci-d13` restored from release 0.14.0, `vc-api`), a signing library (`certify-signing`) shared with a command-line tool (`certify-cli`), MOSIP keymanager kept as the embedded default key provider next to PKCS#11, KMS and file providers, Velocity kept as the default template engine next to a JSON mapping engine, and Flyway-managed additive migrations. Full design: `docs/design/`.
...
2. Keymanager stays an embedded library and the default provider. It is wrapped as `certify-keyprovider-keymanager`; it is never called over HTTP; `io.mosip.kernel` imports live only in that module once WP P1-02 lands. Existing deployments keep the same jar, properties, tables, `kid` values and signature bytes.
...
| Keys | keymanager wiring in `CertifyServiceApplication`, `config/AppConfig`, `services/JwksServiceImpl`, `utils/DIDDocumentUtil` | `certify-keyprovider-keymanager` (default), `-jca`, `-pkcs11`, `-kms-*` |
