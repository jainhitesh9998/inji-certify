...
/** EC_SECP256K1_2019: detached JWS over the URDNA2015 hash through the key provider the configuration's key columns name (P1-03). */
@Component
public class EcdsaSecp256k1Signature2019ProofGenerator implements ProofGenerator {
    ...
    public LdProof generateProof(LdProof vcLdProof, String vcEncodedHash, Map<String, String> keyID) {
        ...
        SigningKey key = provider.resolve(ref).withAlgorithm(SignatureAlgorithm.ES256K);
