...
public class StatusListCredentialServiceTest {
    ...
    public void findOrCreateStatusList_FindsExisting() {
        StatusListCredential slc = new StatusListCredential();
        slc.setId("id6");
        when(statusListCredentialRepository.findFirstByCredentialTypeAndStatusPurposeAndCredentialStatusOrderByCreatedDtimesDesc(eq(StatusListCredentialService.BITSTRING_CREDENTIAL_TYPE), anyString(), any())).thenReturn(Optional.of(slc));
    ...
    public void findOrCreateStatusList_CreatesNew() {
        when(statusListCredentialRepository.findFirstByCredentialTypeAndStatusPurposeAndCredentialStatusOrderByCreatedDtimesDesc(eq(StatusListCredentialService.BITSTRING_CREDENTIAL_TYPE), anyString(), any())).thenReturn(Optional.empty());
        W3CJsonLD w3CJsonLD = mock(W3CJsonLD.class);
        when(credentialFactory.getCredential(VCFormats.LDP_VC)).thenReturn(Optional.of(w3CJsonLD));
        VCResult mockVcResultLdp = new VCResult<JsonLDObject>();
        JsonLDObject signedCredObj = JsonLDObject.fromJson("{\"signed\":\"credential\", \"proof\":{}}");
