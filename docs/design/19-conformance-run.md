...
- The suite fetches the issuer metadata where OpenID4VCI 1.0 section 12.2.2 puts it: `/.well-known/openid-credential-issuer` inserted between host and path, so for the identifier `https://<host>/v1/certify/oid4vci` it requests `https://<host>/.well-known/openid-credential-issuer/v1/certify/oid4vci`, and the authorization server metadata at `https://<host>/.well-known/oauth-authorization-server`. Certify serves both under its servlet path; the proxy in front maps them (`docker-compose/docker-compose-injistack/certify-nginx.conf` has the locations). Point the public host at the proxy, not at port 8090.
